...
async def send_alert(
    ...
    message = (
        f"Phlebotomist #{phlebotomist_id} has exited the designated safety zone as of {datetime.utcnow().isoformat()} UTC."
    )
...
def query_location(phlebotomist_id: int) -> LocationResponse | None:
    ...
    try:
        ...
        resp = requests.get(
            f"{PHLEBOTOMIST_API_BASE}/clinicianstatus/{phlebotomist_id}",
            timeout=5
        )
        ...
        if resp.status_code == 200:
            ...
            polygon_points = [Location(lat=point[0], lon=point[1]) for point in data["features"][1]["geometry"]["coordinates"][0]]
...
def check_clinician_within_bounds(location: Location, bounds: Bounds) -> Union[ClinicianStatus.WITHIN_BOUNDS, ClinicianStatus.OUT_OF_BOUNDS]:
